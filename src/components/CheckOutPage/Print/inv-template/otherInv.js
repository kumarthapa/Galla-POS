    ...
    getTotalSavedAmount = () => {
        ...
        if (billingData && billingData.data) {
            billingData.data.forEach(item => {
                ...
                let price = (CartHelper.isApplyingDiscountAfterTax()) ? item.finalprice : item.price;
                total += Number(price) * Number(item.qty);
            })
        }
                        ...
                        <tbody>
                            ...
                            {billingData.data.map(product => (
                                <tr key={product.id} className="inv-table-row">
                                    ...
                                    <td className="inv-table-data inv-table-data-right" colSpan={2}>
                                        <Typography className="custom-font" variant="body2" component="b">{CartHelper.getItemDisAmt(product)}</Typography>
                                    </td>
                                    <td className="inv-table-data inv-table-data-right" colSpan={2}>
                                        <Typography className="custom-font" variant="body2" component="b">{CartHelper.getItemPrice(product)}</Typography>
                                    </td>
                                </tr>
                            ))}
                            ...
                            {Number(totalSavedAmt) > 0 ?
                                <tr className="inv-table-row">
                                    ...
                                    <td className="inv-table-data align-right" colSpan={4}>
                                        <Typography className="custom-font" variant="subtitle2" component="b">{CartHelper.getCurrencyFormatted(totalSavedAmt.toFixed(2))}</Typography>
                                    </td>
                                </tr>
                                : null}
